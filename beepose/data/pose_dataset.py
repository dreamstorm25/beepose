### Adapted and modified  from ildoonet/tf-pose-estimation
### Apache license 2.0
...
from beepose.data.pose_augmenter import pose_to_img, get_augmented_image
from numba import jit

from beepose.utils.util import get_skeleton_from_json
...
class CocoMetadata:
    ...
    def __init__(self, idx, img_url, img_meta, annotations, sigma=8.0,output_shape=(480,600),numparts=5,coco_vecs = list(zip(
        ...
        self.joint_list = []
        transform = list(zip([i for i in range(numparts)],[i for i in range(numparts)]))
...
#             print("init: ", self.joint_list)

        # logger.debug('joint size=%d' % len(self.joint_list))

    @jit
    def get_heatmap(self, target_size=None):
        heatmap = np.zeros((self.__coco_parts, self.height, self.width), dtype=np.float32)
#         print(self.joint_list)
    ...
    @jit
    def get_vectormap(self, target_size,debug=False):
        vectormap = np.zeros((self.__coco_parts*2, self.height, self.width), dtype=np.float32)
        countmap = np.zeros((self.__coco_parts, self.height, self.width), dtype=np.int16)
        
        
        for joints in self.joint_list:
            if len(joints)<2:continue 
           
            for plane_idx, (j_idx1, j_idx2) in enumerate(self.__coco_vecs):
                j_idx1 -= 1
                j_idx2 -= 1
                if debug:print(joints,j_idx1,j_idx2)
                #if j_idx2>3:continue
                center_from = joints[j_idx1]
                center_to = joints[j_idx2]
                #if center_from[0]>2000 or center_from[1]>2000:
                #    continue 
                if center_from[0] < -10 or center_from[1] < -10 or center_to[0] < -10 or center_to[1] < -10:
                    continue
                if debug:print(len(joints),j_idx1,j_idx2,'PUT VECTOR MAP')
        ...
        if target_size:
            vectormap = cv2.resize(vectormap, target_size, interpolation=cv2.INTER_CUBIC)
...
class CocoPose(RNGDataFlow):
    ...
    def __init__(self, path, img_path=None, is_train=True, decode_img=True, only_idx=-1,numparts=6,
        ...
        #if is_train:
        #    whole_path = path#os.path.join(path, 'person_keypoints_train2017.json')
        #else:
        #    whole_path =  #os.path.join(path, 'person_keypoints_val2017.json')
        self.img_path = img_path#(img_path if img_path is not None else '') + ('train2017/' if is_train else 'val2017/')
    ...
    def get_masks(self,annotations):
    ...
    def get_data(self):
        ...
        #else:
        #    pass
        ## Making an infinite loop to avoid exception when object is depleted
        #while True:
        while True:
            ...
            for idx in idxs:
                ...
                mask_all,mask_miss = self.get_masks(anns)
                meta = CocoMetadata(idx, img_url, img_meta, anns, sigma=self.sigma,
...
def read_image_url(metas):
    for meta in metas:
        ...
        meta.img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    return metas
...
def get_dataflow_batch(path, is_train=True, batch_size=10, img_path=None,sigma=8.0,output_shape=(480,600),
                                   numparts=5,translation=False,scale=False,rotation=True,
                                                mins=0.25,maxs=1.2,mina=-np.pi,maxa=np.pi, ilumination=0.0,image_type='RGB'):
